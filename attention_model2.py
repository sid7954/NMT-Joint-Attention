...
def create_attention_mechanism(attention_option, num_units, memory,
  ...
  # Mechanism
  def sparse_max_safe(scores):
      norm_val = tf.reduce_min(tf.where( (tf.is_finite(scores)),
              scores , -tf.log(tf.constant(0., dtype=scores.dtype))*tf.ones_like(scores)), axis=1, keep_dims=True)
      sparsemax = tf.contrib.sparsemax.sparsemax(tf.where(tf.is_finite(scores), scores, -5 +  norm_val*tf.ones_like(scores)))
      #sparsemax = tf.where(tf.is_finite(scores), sparsemax, tf.zeros_like(sparsemax))
      return sparsemax #/(1e-3 + tf.reduce_sum(sparsemax, axis=1, keep_dims=True))
  ...
  if attention_option == "luong":
    ...
    attention_mechanism = tf.contrib.seq2seq.LuongAttention(
        ...
        probability_fn=sparse_max_safe,
        scale=True)
