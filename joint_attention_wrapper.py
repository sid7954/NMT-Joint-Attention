...
class AttentionWrapperState(
    collections.namedtuple("AttentionWrapperState",
                           ("cell_state", "attention", #"pstr_attention", 
                            "time", "alignments",
                            "alignment_history", "pstr_cell_state", "cell_output"))):
...
def _compute_attention_pstr(pstr_attention_mechanism, cell_output, previous_alignments,
                       attention_layer, pstr_cell_output, attention_mechanism_p):
  """Computes the attention and alignments for a given attention_mechanism."""
  alignments = pstr_attention_mechanism(
      pstr_cell_output, previous_alignments=tf.stop_gradient(previous_alignments))

  # Reshape from [batch_size, memory_time] to [batch_size, 1, memory_time]
  expanded_alignments = array_ops.expand_dims(alignments, 1)

  alignments = tf.verify_tensor_all_finite( alignments, msg="pstr_alignments failed")

  #cell_output = array_ops.tile( array_ops.expand_dims(cell_output,1), [1, tf.shape(attention_mechanism_p.values)[1], 1])
  #if attention_layer is not None:
  #  attended_layers = attention_layer(array_ops.concat([cell_output, attention_mechanism_p.values], 2))
  #else:
  #  attended_layers = attention_mechanism.values

  
  context = math_ops.matmul(expanded_alignments, tf.stop_gradient(attention_mechanism_p.values))
  #context_p = math_ops.matmul(tf.stop_gradient(expanded_alignments), attention_mechanism_p.values)
  context_p = math_ops.matmul(expanded_alignments, attention_mechanism_p.values)

  context = array_ops.squeeze(context, [1])
  context_p = array_ops.squeeze(context_p, [1])


  if attention_layer is not None:
    attention_p = attention_layer(array_ops.concat([cell_output, context_p], 1))
    attention = tf.matmul( array_ops.concat([tf.stop_gradient(cell_output), context],1) , tf.stop_gradient(attention_layer.kernel))

  else:
    attention = context
    attention_p = context_p

  return attention, alignments, attention_p
...
class AttentionWrapper(rnn_cell_impl.RNNCell):
  ...
  def __init__(self,
    ...
    **NOTE** If you are using the `BeamSearchDecoder` with a cell wrapped in
    ...
    if attention_layer_size is not None:
      ...
      self._pstr_attention_layers = tuple(
          layers_core.Dense(
              attention_layer_size, name="pstr_attention_layer", use_bias=False)
          for attention_layer_size in attention_layer_sizes)
      self._pstr_attention_layer_size = sum(attention_layer_sizes)
    ...
    self._pstr_cell_input_fn = pstr_cell_input_fn
    self._pstr_cell = posterior_cell
    self._pstr_attention_mechanism = pstr_attention_mechanism
  ...
  def state_size(self):
    ...
    return AttentionWrapperState(
        ...
        #pstr_attention=self._attention_layer_size,   --there in joint_attention_wrapper
        alignments=self._item_or_tuple(
            a.alignments_size for a in self._attention_mechanisms),
        alignment_history=self._item_or_tuple(
            () for _ in self._attention_mechanisms),  # sometimes a TensorArray
        pstr_cell_state=self._pstr_cell.state_size,
  ...
  def zero_state(self, batch_size, dtype):
    ...
    **NOTE** Please see the initializer documentation for details of how
    ...
    with ops.name_scope(type(self).__name__ + "ZeroState", values=[batch_size]):
      ...
      pstr_cell_state = self._pstr_cell.zero_state(batch_size, dtype)
      with ops.control_dependencies(
          self._batch_size_checks(batch_size, error_message)):
        pstr_cell_state = nest.map_structure(
            lambda s: array_ops.identity(s, name="checked_pstr_cell_state"),
            pstr_cell_state)
      ...
      return AttentionWrapperState(
          ...
          attention=_zero_state_tensors(self._attention_layer_size, batch_size,
                                        dtype),
          #pstr_attention=_zero_state_tensors(self._attention_layer_size, batch_size,
          #                              dtype), -- was there in joint_attn_wrapper.py
          alignments=self._item_or_tuple(
          ...
          pstr_cell_state = pstr_cell_state,
          cell_output=_zero_state_tensors(self._cell.output_size, batch_size, dtype))
  ...
  def call(self, inputs, state):
    ...
    if self._is_training:
        raw_inputs = tf.cast(inputs[:,-1], dtype=tf.int32)
        inputs = inputs[:,:-1]

    cell_inputs = self._cell_input_fn(inputs, state.attention)
    cell_state = state.cell_state
    cell_output, next_cell_state = self._cell(cell_inputs, cell_state)
    prev_cell_output = state.cell_output

    with variable_scope.variable_scope("pstr_attention"):
      pstr_cell_inputs = self._pstr_cell_input_fn(inputs, state.attention)
      pstr_cell_state = state.pstr_cell_state
      pstr_cell_output, next_pstr_cell_state = self._pstr_cell(pstr_cell_inputs, pstr_cell_state)
    ...
    _, pstr_alignments, pstr_attention_p = _compute_attention_pstr(self._pstr_attention_mechanism, prev_cell_output, previous_alignments[0], 
            self._attention_layers[0], pstr_cell_output, self._attention_mechanisms[0])

    #pstr_alignments_stop = pstr_alignments
    #tf.stop_gradient(pstr_alignments) # Should i do stop gradient here???
    #prev_align_stop = tf.stop_gradient(previous_alignments[0])
    #(previous_alignments[0])
    #tf.stop_gradient(previous_alignments[0])

    #KL error
    posterior_kl_error_q = (  math_ops.reduce_sum(pstr_alignments*math_ops.log( 1e-12 + ((pstr_alignments)/(1e-12 + previous_alignments[0]))), axis=-1))

    posterior_kl_error_p = (  math_ops.reduce_sum( previous_alignments[0]*math_ops.log( 1e-12 + ((previous_alignments[0])/(1e-12 + pstr_alignments ))), axis=-1))
    ...
    next_state = AttentionWrapperState(
        ...
        attention=attention,
        #pstr_attention=pstr_attention,
        alignments=self._item_or_tuple(all_alignments),
        alignment_history=self._item_or_tuple(all_histories),
        pstr_cell_state=next_pstr_cell_state,
        cell_output=cell_output,
        )

    batch_size = ( inputs.shape[0].value or array_ops.shape(inputs)[0])

    #self._output_attention is True so follow this path
    if self._output_attention:
      #output = self._output_layer(attention)
      pstr_output = self._output_layer(pstr_attention_p)
      # pstr_output = tf.matmul(pstr_attention, tf.stop_gradient(self._output_layer.kernel))
      # pstr_output_p = self._output_layer(pstr_attention_p)
      
      # output = tf.verify_tensor_all_finite( output, msg="Output Logits failed")
      # output = tf.nn.log_softmax(output)

      # output = tf.verify_tensor_all_finite( output, msg="Output LogLogits failed")
      
      #output = math_ops.reduce_mean(all_logits, axis=0)
      output = all_logits[0]
      if self._is_training:
        """ Compute P(y|pstr) and add it to KL """
        pstr_log_prob = tf.nn.log_softmax(pstr_output)
        pstr_log_prob = tf.verify_tensor_all_finite( pstr_log_prob, msg="Posterior LogLogits failed")
        batch_idx = tf.concat(values=[tf.reshape(tf.range(batch_size),[batch_size,1]), tf.reshape(raw_inputs,[batch_size,1])], axis=1)
        #pstr_log_loss = tf.reshape(tf.gather_nd(-pstr_log_prob, batch_idx),[-1])
        #batch_idx = tf.Print(batch_idx, [tf.shape(batch_idx), tf.shape(pstr_log_prob), tf.shape(pstr_log_loss)], message="BLAHAPE", first_n=10)
        pstr_log_loss = tf.reshape(tf.gather_nd(-pstr_log_prob, batch_idx),[-1])


        #pstr_log_prob_p = tf.nn.log_softmax(pstr_output_p)
        #pstr_log_loss_p = tf.reshape(tf.gather_nd(-pstr_log_prob_p, batch_idx),[-1])

        #output = array_ops.concat(values=[output, array_ops.zeros_like(array_ops.expand_dims(0*posterior_kl_error + pstr_log_loss,1))], axis=1)
        output = array_ops.concat(values=[output, array_ops.expand_dims(pstr_log_loss, 1), 
            array_ops.expand_dims(pstr_log_loss,1), array_ops.expand_dims((posterior_kl_error_p ),1), array_ops.expand_dims( posterior_kl_error_q, 1) ], axis=1)
        """Last index of output used to calculate seperate loss"""
        #output = tf.Print(output1, [tf.shape(posterior_error), tf.shape(output1), posterior_error], message="OUTPUT", first_n=10)
