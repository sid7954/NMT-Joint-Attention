...
def _compute_attention_full(attention_mechanism, cell_output, previous_alignments,
  ...
  #  cell_output = array_ops.tile( array_ops.expand_dims(cell_output,1), [1, tf.shape(attention_mechanism.values)[1], 1])

  if attention_layer is not None:
    attention = attention_layer(avg_attention_vector) #array_ops.concat([cell_output, context], 2))
    if output_layer is not None:
      temp_k=tf.minimum(4,tf.shape(alignments)[1])
      #temp_k=tf.shape(alignments)[1]
      ...
      exp_attention = attention_layer(array_ops.concat([cell_output, top_encoder_parts], 2))
      exp_attention = tf.verify_tensor_all_finite( exp_attention, msg="Exp attn failed")
      exp_logits = output_layer(exp_attention)
      alignments = tf.verify_tensor_all_finite( alignments, msg="Alignments failed")
      exp_logits = tf.verify_tensor_all_finite( exp_logits, msg="Prelogits failed")

      avg_logits=output_layer(attention_layer(avg_attention_vector)) #[batch_size,vocab_size]

      top_prob=tf.reduce_sum(top_alignments.values,1)


      #logits = tf.Print(logits, [tf.shape(alignments), tf.shape(logits), tf.shape(cell_output), tf.shape(attention_mechanism.values)], message="LLL")
      exp_logits = (math_ops.reduce_sum(array_ops.expand_dims(top_alignments.values, 2)*tf.nn.softmax(exp_logits), axis=1))
      logits=exp_logits #+ tf.expand_dims((1.0 -top_prob),1)*tf.nn.softmax(avg_logits)
      logits = tf.verify_tensor_all_finite( logits, msg="Logits failed")
      logits = tf.log(tf.nn.relu(logits) + 1e-12)
...
def _compute_attention_full_pstr(pstr_attention_mechanism, attention_mechanism, pstr_cell_output, cell_output, previous_alignments,
                       attention_layer, output_layer=None):
  """Computes the attention and alignments for a given attention_mechanism."""
  alignments = pstr_attention_mechanism(
      pstr_cell_output, previous_alignments=previous_alignments)
  ...
  if attention_layer is not None:
    attention = attention_layer(avg_attention_vector) #array_ops.concat([cell_output, context], 2))
    if output_layer is not None:
      temp_k=tf.minimum(6,tf.shape(alignments)[1])
      encoder_shape=tf.shape(attention_mechanism.values)
      ...
      #logits = tf.Print(logits, [tf.shape(alignments), tf.shape(logits), tf.shape(cell_output), tf.shape(attention_mechanism.values)], message="LLL")
      exp_logits = (math_ops.reduce_sum(array_ops.expand_dims(top_alignments.values, 2)*tf.nn.log_softmax(exp_logits), axis=1))
      logits = exp_logits #+ tf.expand_dims((1.0 -top_prob),1)*tf.nn.log_softmax(avg_logits)
      logits = tf.verify_tensor_all_finite( logits, msg="Logits failed")
...
class AttentionWrapper(rnn_cell_impl.RNNCell):
  ...
  def call(self, inputs, state):
    ...
    if self._is_training:
        input_tok = tf.cast(inputs[:,-1], tf.int32)
        inputs = inputs[:,:-1]
    ...
    for i, attention_mechanism in enumerate(self._attention_mechanisms):
      attention, alignments,logits  = _compute_attention_full(
    ...
    _, pstr_alignments, pstr_logP = _compute_attention_full_pstr(self._pstr_attention_mechanism, self._attention_mechanisms[0], pstr_cell_output, prev_cell_output, previous_alignments[0], self._attention_layers[0], output_layer=self._output_layer)

    posterior_error_q =  tf.expand_dims(  math_ops.reduce_sum(pstr_alignments*math_ops.log( (1e-6+pstr_alignments)/(1e-6 + previous_alignments[0])), axis=-1), 1)
    posterior_error_p =  tf.expand_dims(  math_ops.reduce_sum(previous_alignments[0]*math_ops.log( (1e-6+previous_alignments[0])/(1e-6 + pstr_alignments)), axis=-1), 1)
    ...
    if self._output_attention:
      ...
      output = all_logits[0]
      if self._is_training:
        ext = array_ops.zeros_like(posterior_error_q)

        batch_idx = tf.concat(values=[tf.reshape(tf.range(batch_size),[batch_size,1]), tf.reshape(input_tok,[batch_size,1])], axis=1)
        pstr_log_loss = tf.reshape(tf.gather_nd(-pstr_logP, batch_idx),[batch_size, 1])


        output = array_ops.concat(values=[output, pstr_log_loss, ext, posterior_error_q, posterior_error_p], axis=1) # switch for p, q for earlier version

        #output = array_ops.concat(values=[output, array_ops.expand_dims(posterior_error,1)], axis=1)
